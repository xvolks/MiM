 ...
 * @since 2.0
 * @version $Id: Entities.java,v 1.2 2007-08-27 13:05:41 fch Exp $
 */
internal class Entities {

    // package scoped for testing
    var map: EntityMap = Entities.LookupEntityMap()
    ...
    internal open class PrimitiveEntityMap : EntityMap {
        ...
        private val mapValueToName = HashMap<Int, String>()
        ...
        fun names(): Collection<String> {
            return mapValueToName.keys.sorted().map { mapValueToName[it] ?: throw IllegalAccessError() }
        }
    }
    ...
    internal class LookupEntityMap : PrimitiveEntityMap() {
        private var lookupTable: Array<String> = createLookupTable()
         ...
         *
         */
        private fun createLookupTable(): Array<String> {
            var i = 0
            lookupTable = names().filter { i++<LOOKUP_TABLE_SIZE }.map { it }.toTypedArray()
            return lookupTable
        }
    }
     ...
     *
     * @param writer
     * The `Writer` to write the results of the escaping to. Assumed to be a non-null value.
     * @param str
     * The `String` to escape. Assumed to be a non-null value.
     * @throws IOException
     * when `Writer` passed throws the exception from calls to the [Writer.write]
     * methods.
     *
     * @see .escape
     * @see Writer
        ...
        while (i < len) {
            ...
            if (c == '&') {
                ...
                if (entityContentLen > 0) {
                    if (entityContent.get(0) == '#') { // escaped value content is an integer (decimal or
                        // hexidecimal)
                        if (entityContentLen > 1) {
                            val isHexChar = entityContent.get(1)
                            try {
                                when (isHexChar) {
                                    'X', 'x' -> {
    ...
    companion object {
        ...
        // package scoped for testing
        val ISO8859_1_ARRAY = arrayOf(arrayOf("nbsp", "160"), // non-breaking space
